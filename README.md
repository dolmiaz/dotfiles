...
デフォルトでは実体コピーで配置します。symlink で配置したい場合は `--link` を指定します。
...
VS Code は apt / dnf / yum 環境では Microsoft の package repository を追加して `code` を導入します。macOS では Homebrew cask の `visual-studio-code` を導入します。既に `code` command がある場合はスキップします。
...
- 実体コピー配置で既に同じ内容のファイルがある場合はスキップします。
